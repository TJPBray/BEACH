...
% Last Modified by GUIDE v2.5 17-Jul-2020 14:31:17
...
% --- Executes on button press 'Define normal bone'.
...
hroi = draw_normroi(handles,UD.ha) ; 
...
UD.roislice(nroi+1) = islice ;
UD.hroi{nroi+1} = hroi ; %UD.hroi{} is a list of polygons
UD.roilabel{nroi+1} = strcat(labstr,' Left');
...
% --- Executes on button press in Freehand ROI.
...
[hroi_1] = freehand(handles,UD.ha) ; %hroi_this is a polygon
...
function hroi = freehand(handles, varargin)
...
pvmedian=median(pv);
pv95th=prctile(pv,95)
pvmax=max(pv);
IQR=(prctile(pv,75))-(prctile(pv,25))
meanpv=(mean(pv))

%Calculate key ADC values
ADCmean=nanmean(pv);
Min=min(pv);
ADC10=prctile(pv,10);
ADC25=prctile(pv,25);
ADCmedian=prctile(pv,50);
ADC75=prctile(pv,75);
ADC90=prctile(pv,90);
Max=max(pv);
...
FDFfixed=double(abovethreshpixels/totalpixels);
...
HistData(1,1)=ADCmean;
HistData(1,2)=ADCmedian;
HistData(1,3)=ADC10;
HistData(1,4)=ADC25;
HistData(1,5)=ADC75;
HistData(1,6)=ADC90;
...
%Print data for copy/paste to excel
HistData=HistData.'

%Create column names
Param={'ADCmean';'ADCmedian';'ADC10';'ADC25';'ADC75';'ADC90';'Max';'Min'};
...
hist(double(pv),nbins)
hhist = gca ;
hold on

yl=ylim;
plot([N95 N95],[yl(1) yl(2)])
...
t.Position= [200 0 190 160]

set(handles.edit_threshold,'String',num2str(N95)) 
...
BEACH_ADC('popupmenu_activefig_Callback',handles.popupmenu_activefig, [], handles) ;
