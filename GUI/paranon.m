...
% PARANON Anonymise Philips PAR/REC files
% Usage:  paranon   - brings up GUI.
%   First close all applications that use the PAR or REC files to be anonymised.
%   Selects folder containing PAR & REC files of one (and only one) subject.
%   Enter suitable name for anonymisation, a name based on the time
%   and date will have been suggested.
%   Modify the Examination Date /Time
%   Modify the path of the Dataset Name that is written into the
%   PAR file. 
...
%     The original is then overwritten wit the temporary copy.
...
%  possible with this information and access to PACs. In many situations
%  this can be beneficial as anonymity is high for all without PACS access,
%  but subjects may be traced back if clinically necessary.
%  * The PAR and REC file names may contain the patient name.
%  * The original PAR and REC file name, without the .PAR or .REC
%  extension, but with the complete path, is written into the PAR file.
%  Called the Dataset Name
%
...
% Last Modified by GUIDE v2.5 24-Jul-2012 21:15:29
...
set(handles.edit_patientname,'String',['anon',str]) ;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
folder_name = uigetdir('','Select PAR/REC folder') ;
if folder_name == 0
...
set(handles.edit_directory,'String',folder_string)
...
% Hints: get(hObject,'String') returns contents of edit_patientname as text
%        str2double(get(hObject,'String')) returns contents of edit_patientname as a double
...
function update_namelist(handles) 
  ...
  new_pn = get(handles.edit_patientname,'String') ;
  ...
  set(handles.edit_examdate,'String',parv.ExamDate) ;
...
anon_pn = get(handles.edit_patientname,'String') ;
...
str = [get(handles.text_patientname,'String'), ' , ', ...
       get(handles.edit_patientname,'String')] ;
