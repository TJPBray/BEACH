%NB: Only suitable for vertical joints at present
...
%Draw polyline
imshow(im,[]);
roi=impoly('Closed',false)
polypoints=roi.getPosition
...
xlength(k,1)=(length^2/(1+perpgrad(k,1)^2))^0.5 %Gives length in x and y direction of perplines
...
xleng=(length^2/(1+perpgrad1^2))^0.5 %Gives length in x and y direction of perplines
...
xleng=(length^2/(1+perpgradl^2))^0.5 %Gives length in x and y direction of perplines
...
%Create polygon
xy2inv=flipud(xy2) %inverts xy2
xy=[xy1; xy2inv] %vertically concatenates all point
ROI=impoly(gca,xy,'Closed',true)
setColor(ROI,'red');
